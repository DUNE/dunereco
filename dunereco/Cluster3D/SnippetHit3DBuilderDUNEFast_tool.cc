/**
 *  @file   SnippetHit3DBuilderDUNEFast_tool.cc
 *
...
/**
 *  @brief  SnippetHit3DBuilderDUNEFast class definiton
 */
class SnippetHit3DBuilderDUNEFast : virtual public IHit3DBuilder
{
public:
    /**
     *  @brief  Constructor
     ...
     *  @param  pset
     */
    explicit SnippetHit3DBuilderDUNEFast(fhicl::ParameterSet const &pset);
...
private:
     ...
     */
    using MatchedHitMap = std::unordered_map<const reco::ClusterHit2D*,std::unordered_set<const reco::ClusterHit2D*>>;
    ...
    bool makeHitTriplet(reco::ClusterHit3D&       pairOut,
                        ...
                        const reco::ClusterHit2D* hit2,
                        reco::ClusterHit2DVec*    hitVectorOut = nullptr) const;
    ...
    float                                   m_deltaPeakTimeSigLongHit; ///< Separate significance for pairs involving a "long hit" (pulse train) segment,
                                                                        ///< whose RMS is a segment-coverage proxy, not a fit uncertainty -- see m_LongHitStretchFctr
    float                                   m_rangeNumSig;
    ...
    size_t                                  m_maxMythicalPointsPerWire; ///< Cap on 2-hit "mythical" points sharing a third-plane wire target --
                                                                        ///< unlike triplets (capped via numToKeep in findGoodTriplets), 2-hit points
                                                                        ///< had no such bound, which dominates output volume in busy events
...
SnippetHit3DBuilderDUNEFast::SnippetHit3DBuilderDUNEFast(fhicl::ParameterSet const &pset) :
...
void SnippetHit3DBuilderDUNEFast::produces(art::ProducesCollector& collector)
{
...
void SnippetHit3DBuilderDUNEFast::configure(fhicl::ParameterSet const &pset)
{
    ...
    m_deltaPeakTimeSigLongHit = pset.get<float                  >("DeltaPeakTimeSigLongHit", m_deltaPeakTimeSig);
    ...
    m_maxMythicalPointsPerWire = pset.get<size_t                >("MaxMythicalPointsPerWire", 16);
    ...
    if (m_outputHistograms)
    {
        ...
        m_tupleTree = tfs->make<TTree>("Hit3DBuilderTree", "Tree by SnippetHit3DBuilderDUNEFast");
...
void SnippetHit3DBuilderDUNEFast::clear()
{
    ...
    m_deltaPeakTimePlane2Vec.clear();
    m_deltaPeakSigmaPlane2Vec.clear();
...
void SnippetHit3DBuilderDUNEFast::Hit3DBuilder(art::Event& evt, reco::HitPairList& hitPairList, RecobHitToPtrMap& clusterHitToArtPtrMap)
{
    // Clear the internal data structures
    m_clusterHit2DMasterList.clear();
    m_planeToSnippetHitMap.clear();

    mf::LogDebug("SnippetHit3D") << "SnippetHit3DBuilderDUNEFast entered" << std::endl;
    ...
    mf::LogDebug("SnippetHit3D") << "SnippetHit3DBuilderDUNEFast building space points";

    // If there are no hits in our view data structure then do not proceed with the full analysis
    if (!m_planeToSnippetHitMap.empty())
    {
    ...
    mf::LogDebug("SnippetHit3D") << "SnippetHit3DBuilderDUNEFast exited";

    return;
}

void SnippetHit3DBuilderDUNEFast::BuildHit3D(reco::HitPairList& hitPairList) const
{
    /**
...
size_t SnippetHit3DBuilderDUNEFast::BuildHitPairMap(PlaneToSnippetHitMap& planeToSnippetHitMap, reco::HitPairList& hitPairList) const
{
    /**
...
size_t SnippetHit3DBuilderDUNEFast::BuildHitPairMapByTPC(PlaneSnippetHitMapItrPairVec& snippetHitMapItrVec, reco::HitPairList& hitPairList) const
{
    /**
     *  @brief Given input 2D hits, build out the lists of possible 3D hits
     *
...
int SnippetHit3DBuilderDUNEFast::findGoodHitPairs(SnippetHitMap::iterator& firstSnippetItr,
...
void SnippetHit3DBuilderDUNEFast::findGoodTriplets(HitMatchTripletVecMap& pair12Map, 
                                              HitMatchTripletVecMap& pair13Map, 
                                              MatchedHitMap&         matchedHitMap,
    ...
    if (!pair12Map.empty() && !pair13Map.empty())
    {
        ...
        for(const auto& pair12 : pair12Map)
        {
            ...
            for(const auto& hit2Dhit3DPair12 : pair12.second)
            {
                ...
                std::vector<TripletCandidate> localCandidates;
                localCandidates.reserve(32);
                ...
                for(const auto& pair13 : pair13Map)
                {
                    ...
                    for(const auto& hit2Dhit3DPair13 : pair13.second)
                    {
                        ...
                        if (makeHitTriplet(triplet, matchedHitMap, pair1, hit2, &hitVector))
                        {
                            float timeSpread(0.f);
                            const float avePeakTime = triplet.getAvePeakTime();

                            for (const auto* hit2D : triplet.getHits())
                            {
                                if (!hit2D) continue;
                                const float delta = std::fabs(hit2D->getTimeTicks() - avePeakTime);
                                if (delta > timeSpread) timeSpread = delta;
                            }

                            const float overlapFraction = std::max(0.f, std::min(1.f, triplet.getOverlapFraction()));
                            const float score = triplet.getHitChiSquare()
                                               + 5.f * std::abs(triplet.getChargeAsymmetry())
                                               + 0.25f * timeSpread
                                               + (1.f - overlapFraction);
                            localCandidates.emplace_back(TripletCandidate{triplet, hitVector, score});
                        }
                    }
                }

                // Sort indices rather than TripletCandidate values in place: reco::ClusterHit3D has a
                // hand-written copy-assignment operator (copy-and-swap idiom) but no move constructor, so
                // std::swap<ClusterHit3D> falls back to that same operator= internally and recurses into
                // itself forever -- reordering TripletCandidate (which holds a ClusterHit3D by value) via
                // std::sort segfaults with a stack overflow the moment two elements are out of order.
                // Swapping size_t indices during the sort avoids ever assigning a ClusterHit3D in place;
                // the single copy made by push_back below (copy-construction, not assignment) is safe.
                std::vector<size_t> sortedIdx(localCandidates.size());
                std::iota(sortedIdx.begin(), sortedIdx.end(), size_t(0));

                std::sort(sortedIdx.begin(), sortedIdx.end(),
                          [&localCandidates](size_t left, size_t right)
                          {
                              if (localCandidates[left].score != localCandidates[right].score) return localCandidates[left].score < localCandidates[right].score;
                              return localCandidates[left].triplet.getAvePeakTime() < localCandidates[right].triplet.getAvePeakTime();
                          });

                const size_t numToKeep = std::min<size_t>(sortedIdx.size(), 16);
                for (size_t idx = 0; idx < numToKeep; ++idx)
                    tripletCandidates.push_back(localCandidates[sortedIdx[idx]]);
            }
        }

        // Note: a 2D hit legitimately can belong to more than one 3D triplet (crossing tracks, wide/merged
        // hits, busy regions) -- consistent with every other LArSoft Hit3DBuilder in this family, we accept
        // every scored candidate and just flag reused hits via SHAREDINTRIPLET rather than rejecting them.
        // Downstream (SPINE) deghosting handles the resulting duplication; the per-pair12 numToKeep cap above
        // bounds how many triplets a single busy UV pair can spawn.
        for (const auto& candidate : tripletCandidates)
        {
            reco::ClusterHit3D triplet = candidate.triplet;
            const reco::ClusterHit2DVec& hitVector = candidate.hitVector;
            ...
            hitPairList.back().setStatusBit(reco::ClusterHit3D::MADESPACEPOINT);
        }
    }
...
int SnippetHit3DBuilderDUNEFast::saveOrphanPairs(HitMatchTripletVecMap& pairMap, MatchedHitMap& matchedHitMap, reco::HitPairList& hitPairList) const
{
    ...
    if (!pairMap.empty())
    {
        ...
        for(const auto& pair : pairMap)
        {
            ...
            // Unlike triplets (capped per UV pair via numToKeep in findGoodTriplets), 2-hit "mythical"
            // points had no bound on how many could share a third-plane wire target -- in a busy wire
            // that's the dominant source of ghost-point volume. Collect the candidates for this wire
            // target first, then keep only the best m_maxMythicalPointsPerWire by chi-square.
            std::vector<OrphanCandidate> candidates;
            candidates.reserve(pair.second.size());
            ...
            for(const auto& hit2Dhit3DPair : pair.second)
            {
                ...
                if (hit1->WireID().Plane != 2 && hit2->WireID().Plane != 2) continue;

                // Allow cut on the quality of the space point
                if (hit3D.getHitChiSquare() >= m_maxMythicalChiSquare) continue;

                candidates.push_back({hit1, hit2, hit3D, hit3D.getHitChiSquare()});
            }

            // Sort indices rather than OrphanCandidate values in place -- ClusterHit3D's copy-assignment
            // operator recurses infinitely with std::swap (see findGoodTriplets), so this must not be a
            // plain std::sort on the candidates themselves.
            std::vector<size_t> sortedIdx(candidates.size());
            std::iota(sortedIdx.begin(), sortedIdx.end(), size_t(0));

            std::sort(sortedIdx.begin(), sortedIdx.end(),
                      [&candidates](size_t left, size_t right)
                      { return candidates[left].chiSquare < candidates[right].chiSquare; });

            const size_t numToKeep = std::min(sortedIdx.size(), m_maxMythicalPointsPerWire);

            for(size_t idx = 0; idx < numToKeep; ++idx)
            {
                const OrphanCandidate& candidate = candidates[sortedIdx[idx]];

                // Add to the list
                hitPairList.emplace_back(candidate.hit3D);
                hitPairList.back().setID(hitPairList.size()-1);
                matchedHitMap[candidate.hit1].insert(candidate.hit2);
                matchedHitMap[candidate.hit2].insert(candidate.hit1);
            }
        }
    }
...
int SnippetHit3DBuilderDUNEFast::saveBadChannelPairs(HitMatchTripletVecMap& pairMap, MatchedHitMap& matchedHitMap, reco::HitPairList& hitPairList) const
{
    int curTripletCount = hitPairList.size();
...
bool SnippetHit3DBuilderDUNEFast::makeHitPair(reco::ClusterHit3D&       hitPair,
                                         const reco::ClusterHit2D* hit1,
                                         const reco::ClusterHit2D* hit2,
                                         ...
                                         size_t                    hitPairCntr) const
{
    ...
    // A "long hit" (pulse train) segment's RMS is a segment-coverage proxy, not a fit uncertainty in the
    // statistical sense a real Gaussian fit gives a normal hit -- so the significance cut below needs its
    // own, independently-tunable threshold rather than sharing m_deltaPeakTimeSig with fit hits.
    float deltaPeakTimeSig = (hit1NDF < 2 || hit2NDF < 2) ? m_deltaPeakTimeSigLongHit : m_deltaPeakTimeSig;
    ...
    if (fabs(hit1Peak - hit2Peak) <= (hit1Width + hit2Width))
    {
        ...
        if (deltaPeakTime < deltaPeakTimeSig * sigmaPeakTime)    // 2 sigma consistency? (do this way to avoid divide)
        {
...
bool SnippetHit3DBuilderDUNEFast::makeHitTriplet(reco::ClusterHit3D&       hitTriplet,
                                            MatchedHitMap&            matchedHitMap,
                                            const reco::ClusterHit3D& pair,
                                            const reco::ClusterHit2D* hit,
                                            reco::ClusterHit2DVec*    hitVectorOut) const
{
    // Assume failure
    bool result(false);
    ...
    if (fabs(hitTimeTicks - pair.getAvePeakTime()) < m_hitWidthSclFctr * (pair.getSigmaPeakTime() + hitSigma))
    {
        ...
        if (hitWireDist < wirePitch)
        {
            ...
            if (notMatched && makeHitPair(pair0h, hit0, hit, m_hitWidthSclFctr) && makeHitPair(pair1h, hit1, hit, m_hitWidthSclFctr))
            {
                ...
                if (hitVectorOut) *hitVectorOut = hitVector;
                ...
                float minHitWindow(std::numeric_limits<float>::max());
                ...
                for(const auto& hit2D : hitVector)
                {
                    ...
                    // Normalize the overlap fraction below against the tightest (most precise) hit's
                    // own window, not the widest. A "long hit" segment carries an inflated, low-precision
                    // window by construction (see m_LongHitStretchFctr); using the widest window as the
                    // denominator would artificially deflate the overlap fraction for a triplet where the
                    // two normal hits actually agree well, just because the long hit's window is broad.
                    minHitWindow = std::min(float(hitStop - hitStart), minHitWindow);
                }
...
bool SnippetHit3DBuilderDUNEFast::WireIDsIntersect(const geo::WireID& wireID0, const geo::WireID& wireID1, geo::WireIDIntersection& widIntersection) const
{
    bool success(false);
...
float SnippetHit3DBuilderDUNEFast::closestApproach(const Eigen::Vector3f& P0,
...
float SnippetHit3DBuilderDUNEFast::chargeIntegral(float peakMean,
                                           float peakAmp,
                                           float peakSigma,
                                           float areaNorm,
                                           int   low,
...
const reco::ClusterHit2D* SnippetHit3DBuilderDUNEFast::FindBestMatchingHit(const Hit2DSet& hit2DSet, const reco::ClusterHit3D& pair, float pairDeltaTimeLimits) const
{
...
int SnippetHit3DBuilderDUNEFast::FindNumberInRange(const Hit2DSet& hit2DSet, const reco::ClusterHit3D& pair, float range) const
{
    static const float minCharge(0.);
...
geo::WireID SnippetHit3DBuilderDUNEFast::NearestWireID(const Eigen::Vector3f& position, const geo::WireID& wireIDIn) const
{
    int wire(0);
...
float SnippetHit3DBuilderDUNEFast::DistanceFromPointToHitWire(const Eigen::Vector3f& position, const geo::WireID& wireIDIn) const
{
...
bool Hit2DSetCompare::operator() (const reco::ClusterHit2D* left, const reco::ClusterHit2D* right) const
{
    ...
    const auto& leftHit = left->getHit();
    const auto& rightHit = right->getHit();

    const double leftPeakTime = leftHit->PeakTime();
    const double rightPeakTime = rightHit->PeakTime();

    if (leftPeakTime != rightPeakTime) return leftPeakTime < rightPeakTime;

    if (leftHit->Channel() != rightHit->Channel()) return leftHit->Channel() < rightHit->Channel();

    const auto& leftWireID = left->WireID();
    const auto& rightWireID = right->WireID();

    if (leftWireID.Cryostat != rightWireID.Cryostat) return leftWireID.Cryostat < rightWireID.Cryostat;
    if (leftWireID.TPC != rightWireID.TPC) return leftWireID.TPC < rightWireID.TPC;
    if (leftWireID.Plane != rightWireID.Plane) return leftWireID.Plane < rightWireID.Plane;
    if (leftWireID.Wire != rightWireID.Wire) return leftWireID.Wire < rightWireID.Wire;

    return left < right;
}

//------------------------------------------------------------------------------------------------------------------------------------------
void SnippetHit3DBuilderDUNEFast::CollectArtHits(const art::Event& evt) const
{
    /**
    ...
    for (const auto& recobHit : recobHitVec)
    {
        ...
        // Start/End ticks to identify the snippet. Use the same corrected time base as hit matching.
        // The plane ID is defined per wire association, so compute it in the inner loop.
        ...
        for(auto wireID : wireIDs)
        {
            ...
            const double startTick = recobHit->StartTick() - m_PlaneToT0OffsetMap.find(planeID)->second;
            const double endTick   = recobHit->EndTick()   - m_PlaneToT0OffsetMap.find(planeID)->second;

            HitStartEndPair hitStartEndPair(static_cast<raw::TDCtick_t>(std::llround(startTick)),
                                             static_cast<raw::TDCtick_t>(std::llround(endTick)));
...
void SnippetHit3DBuilderDUNEFast::CreateNewRecobHitCollection(art::Event&              event,
                                                            reco::HitPairList&       hitPairList,
                                                            ...
                                                            RecobHitToPtrMap&        recobHitToPtrMap)
{
    ...
    hitPtrVec.reserve(std::min<size_t>(m_clusterHit2DMasterList.size(), hitPairList.size() * 3));
...
void SnippetHit3DBuilderDUNEFast::makeWireAssns(const art::Event& evt, art::Assns<recob::Wire, recob::Hit>& wireAssns, RecobHitToPtrMap& recobHitPtrMap) const
{
...
void SnippetHit3DBuilderDUNEFast::makeRawDigitAssns(const art::Event& evt, art::Assns<raw::RawDigit, recob::Hit>& rawDigitAssns, RecobHitToPtrMap& recobHitPtrMap) const
{
    // Let's make sure the input associations container is empty
...
DEFINE_ART_CLASS_TOOL(SnippetHit3DBuilderDUNEFast)
} // namespace lar_cluster3d

...
local sigoutform = 'dense';  // eg "sparse" or "dense"
...
// local sp = sp_maker(params, tools, { sparse: sigoutform == 'dense' });
...
local frame_tap = function(anode_ident)
    g.fan.tap('FrameFanout',
        g.pnode({
            ...
            data: {
                outname: 'protodune-sp-frames-anode%d.tar.bz2' % anode_ident,
                tags: [
                    'gauss%d'  % anode_ident,

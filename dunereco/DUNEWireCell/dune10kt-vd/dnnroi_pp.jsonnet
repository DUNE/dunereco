// Per-anode per-plane DNN-ROI subgraph for DUNE FD-VD (dune10kt-vd).
//
// Ported from pgrapher/experiment/protodunevd/dnnroi_pp.jsonnet: FD-VD
// uses the ProtoDUNE-VD 6-channel model (same CRP anode design).
//
// Uses two single-plane DNNROIFinding nodes (U, V) sharing the same
// TorchService.  Each call feeds the model an (1, 6, nchan_plane, nticks)
// tensor; the two calls are serialized by the TorchService semaphore so
// peak activation memory is for one plane at a time.
//
// The PDVD .ts was trained on stacked U+V.  The UNet is fully
// convolutional on the channel axis, so per-plane and per-CRM inference
// is structurally compatible.
//
// Per-anode policy: U and V via model, W from standard SP gauss
// passthrough.  All CRMs use the same model for both U and V.
//
...
//
// Output frame trace tags per CRM N:
...
function(anode, ts, prefix='dnnroi',
  ...
  // PDVD ships 6-channel models; order must match the training im_tags
  // exactly.
  assert nchan == 6 : 'dune10kt-vd dnnroi nchan must be 6, got %d' % nchan;
  local apa_intags = [
    'loose_lf%d' % apaid,
    'mp2_roi%d' % apaid,
    'mp3_roi%d' % apaid,
  ...
  local dnn_node(plane, outtag, name_suffix, dbg_suffix) = pg.pnode({
    ...
    data: {
      ...
      // The PDVD 6-ch model has two stride-2 levels on the channel axis,
      // so the channel count must be a multiple of 4.  FD-VD CRMs have
      // 286 U/V strips -> zero-pad to 288 (needs WCT with DNNROIFinding
      // chan_pad_multiple support; ignored by older releases).
      chan_pad_multiple: 4,
      nchunks: nchunks,

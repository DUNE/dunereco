...
//local wcls_input = {
//    depos: wcls.input.depos(name="", art_tag="IonAndScint", assn_art_tag="IonAndScint:priorSCE"),
...
local wcls_depoflux_writer = g.pnode({
  ...
  data: {
    ...
    energy: 0, // energy: 0 means use the true depo energy (equivalent to SimChannelSink
               // use_energy=true). NB: a non-zero value is a CONSTANT energy per depo,
               // so the previous "energy: 1" saved 1 MeV for every deposition.
    ...
    // post-drift depo diffusion). Values = pdhd SP-filter recipe
    // (sp-filters.jsonnet):
    //   smear_long [ticks] = sigma_t/tick, sigma_t = 1/(2*pi*0.12MHz) = 1.326us
    //   smear_tran [pitch] = 1/(2*sqrt(pi)*k): Wire_ind k=0.75 (U,V); Wire_col k=10.0 (W)
    smear_long: 2.6526,
    smear_tran: [0.37612, 0.37612, 0.028209],
  },
...
local graph = g.pipeline([wcls_input.deposet, setdrifter, wcls_depoflux_writer, bi_manifold, retagger, wcls_output.sim_digits, sink]);

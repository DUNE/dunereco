    ...
    for(size_t i=0; i<hcs.size(); i++) {
      ...
      for(size_t j=i+1; j<hcs.size(); j++){
        ...
        if( hcs[i].IntersectLocations.count(hcs[j].ID) ) {
          ...
          const geo::WireID wireA(cryo,tpc,pli,hcs[i].CenterWire);
          const geo::WireID wireB(cryo,tpc,plj,hcs[j].CenterWire);
          geo::Point_t xyz;
          match3d = art::ServiceHandle<geo::Geometry>()->WireIDsIntersect(wireA,wireB,xyz);
          y = xyz.Y();
          z = xyz.Z();
        }
